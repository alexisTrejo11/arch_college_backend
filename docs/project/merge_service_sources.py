...
PLATFORM = {
    ...
    "description": (
        "Production-style Spring Boot microservices monorepo for an architecture faculty. "
        "Seven domain services cover identity and access, student and teacher records, "
        "the academic catalog, class scheduling, enrollments, and grades—each with its own "
        "REST API and PostgreSQL database, JWT authentication, and registration through Eureka "
        "and Spring Cloud Config.\n\n"
        "The platform is documented and runnable as deployed: Docker Compose brings up "
        "Postgres, MongoDB, RabbitMQ, Config Server, Eureka, Spring Boot Admin, and all "
        "business services. MongoDB and RabbitMQ support enrollment preload, grade projections, "
        "and scheduling integration where asynchronous or document-oriented workloads apply."
    ),
}
...
def append_block_scalar(lines: list[str], key: str, text: str, indent: int = 0) -> None:
    prefix = " " * indent
    content_prefix = " " * (indent + 2)
    lines.append(f"{prefix}{key}: |-")
    ...
    for index, paragraph in enumerate(paragraphs):
        if index > 0:
            lines.append(f"{content_prefix}")
        for paragraph_line in paragraph.splitlines():
            lines.append(f"{content_prefix}{paragraph_line}")


def tech_stack_item_value(line: str) -> str | None:
    stripped = line.strip()
    if not stripped.startswith("-"):
        return None
    match = re.match(r'^-\s*"(.*)"\s*$', stripped) or re.match(r"^-\s*'(.*)'\s*$", stripped)
    if match:
        return match.group(1)
    return stripped[1:].strip().strip('"').strip("'")


def deduped_tech_stack(documents: list[SourceDocument]) -> list[str]:
    seen: set[str] = set()
    items: list[str] = []
    for document in documents:
        for line in top_children(document, "techStack"):
            value = tech_stack_item_value(line)
            if not value:
                continue
            key = value.casefold()
            if key in seen:
...
def append_deduped_tech_stack(lines: list[str], documents: list[SourceDocument]) -> None:
    items = deduped_tech_stack(documents)
...
def render_metadata(documents: list[SourceDocument]) -> str:
    ...
    append_block_scalar(lines, "description", PLATFORM["description"])
    append_deduped_tech_stack(lines, documents)
    lines.extend(
        [
...
def merged_body(filename: str, documents: list[SourceDocument]) -> str:
    if filename == "ProjectMetadata.md":
        paragraphs = PLATFORM["description"].split("\n\n")
        lines = [
            f"# {title_for(filename)}",
            "",
            "> Auto-generated by `docs/project/merge_service_sources.py`. "
            "Edit `PLATFORM[\"description\"]` in that script, or per-service "
            "`{service}/docs/source/ProjectMetadata.md` for service-specific metadata.",
            "",
        ]
        lines.extend(paragraphs[0:1])
        if len(paragraphs) > 1:
            lines.append("")
        ...
        for item in deduped_tech_stack(documents):
            lines.append(f"- {item}")
        lines.append("")
        return "\n".join(lines).rstrip() + "\n"
