...
Maintains student master data, queries by identifiers, and coordinates progression events over RabbitMQ. This service is part of the **Architecture College Platform** (deployed stack documented alongside `docker-compose.yml`).
...
- **Role:** Student aggregate and progression events for enrollment and grading workflows.
- **Default port:** `8083` (see root `docker-compose.yml`)
- **Persistence:** PostgreSQL (`student_db`)
- **Integration:** Spring AMQP / RabbitMQ

## Core capabilities

- CRUD operations on students
- Lookup by id, account number, or composite filters
- Assign professional line and modality
- Increment completed semester
...
Representative operations include: Fetch student by id, Fetch by account number, List students, Create student. The full contract is generated at runtime—use Swagger UI or [`docs/generated/APISchema.md`](docs/generated/APISchema.md) for portfolio alignment with `APISchema` in `docs-schema.ts`.
...
The `student-service` container receives datasource, discovery, and (if applicable) Rabbit/Mongo settings inline in Compose.
