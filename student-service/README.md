...
Representative operations include: Fetch student by id, Fetch by account number, List students, Create student. The full contract is generated at runtime—use Swagger UI or `docs/APISchema.md` for portfolio alignment with `APISchema` in `docs-schema.ts`.
