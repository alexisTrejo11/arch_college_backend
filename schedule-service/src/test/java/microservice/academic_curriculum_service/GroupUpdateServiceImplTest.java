...
import microservice.schedule_service.Service.GroupServices.Implementation.GroupValidationService;
...
public class GroupUpdateServiceImplTest {
    ...
    private final String currentSemester = "2025-1";
    ...
    void cancelGroup_ShouldCancelGroupIfEligible() {
        when(groupValidationService.cancelGroupIfEligible(groupKey, currentSemester)).thenReturn(Result.success());

        Result<Void> result = groupUpdateService.cancelGroup(groupKey);

        assertTrue(result.isSuccess());
        verify(groupValidationService).cancelGroupIfEligible(groupKey, currentSemester);
    }

    @Test
    void cancelGroup_GroupNotEligible_ShouldReturnError() {
        when(groupValidationService.cancelGroupIfEligible(groupKey, currentSemester)).thenReturn(Result.error("Error"));
    ...
    void removeTeacherToGroup_ShouldRemoveTeacherAndReturnGroupDTO() {
        ...
        when(groupRepository.findByGroupKeyAndSchoolPeriod(groupKey, currentSemester)).thenReturn(Optional.of(group));
        when(groupMapper.entityToDTO(group)).thenReturn(new GroupDTO());
