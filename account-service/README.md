...
Representative operations include: Register a student account, Register a teacher account, Register an admin account, Authenticate and obtain JWT. The full contract is generated at runtime—use Swagger UI or `docs/APISchema.md` for portfolio alignment with `APISchema` in `docs-schema.ts`.
